...
require 'php_functions/dataset_functions.php';
...
function searchElastic($keyword){
        ...
        $params_datasets = [
                'index' => 'publications_2_0',
                'type' => 'publications_2_0_records',
                'size' => 5,
                'from' => 0,
                'body' => [
                        'query' => [
...
$dataset_metadata = retrieveDataset($id);
$json_geonames_terms = retrieveGeonames($id);
$json_licenses = retrieveLicenses($id);
...
$no_license = empty(json_decode($json_licenses));
...
$summary = $dataset_metadata["dataset"]["summary"];
...
$title = $dataset_metadata["dataset"]["title"];


/*<div class="post-meta__content fa fa-link">
            <a class="post-link text-underline" href="">permalink 1</a>
          </div>*/

$doi = $dataset_metadata["dataset"]["doi"];
$hdl = $dataset_metadata["dataset"]["hdl"];
$url = $dataset_metadata["dataset"]["url"];

$providers = $dataset_metadata["dataset"]["providers"];
...
if ((strpos($url, 'hdl') === false)&&(strpos($url, 'doi') === false)) {

	$permalinks = $permalinks.'<div class="post-meta__content fa fa-link"><a class="post-link" target="_blank" href="'.$url.'">URL</a></div><br>';

}

$license_text = $dataset_metadata["dataset"]["license"];
$terms_of_use_text = $dataset_metadata["dataset"]["terms_of_use"];

/*if(strcmp($license_text,'NONE')==0){
        $license = 'Not Defined';

}else{

	$license = '<a href="" class="toggle-lightbox-4">VIEW</a>';
}*/

if((strcmp($terms_of_use_text,'NULL')==0)||(strlen($terms_of_use_text)<3)){
	$terms_of_use = ' <p class="access-text">Not Defined</p>';

}else{
	$terms_of_use='<a href="" class="toggle-lightbox-3">VIEW</a>';
}
...
if( $no_license == FALSE){
	...
	$license_array = json_decode($json_licenses);

	$cc = '<img alt="Creative Commons License" style="border-width:0; width:22%;" src="https://creativecommons.org/images/deed/svg/cc_blue.svg" />';
	$by = '<img alt="Creative Commons License" style="border-width:0; width:22%;"  src="https://creativecommons.org/images/deed/svg/attribution_icon_blue.svg" />';
	$nc = '<img alt="Creative Commons License" style="border-width:0; width:22%;"  src="https://creativecommons.org/images/deed/svg/nc_blue.svg" />';
	$nd = '<img alt="Creative Commons License" style="border-width:0; width:22%;"  src="https://creativecommons.org/images/deed/svg/nd_blue.svg" />';
	$sa = '<img alt="Creative Commons License" style="border-width:0; width:22%;"  src="https://creativecommons.org/images/deed/svg/sa_blue.svg" />';
	$zero = '<img alt="Creative Commons License" style="border-width:0; width:22%;"  src="https://creativecommons.org/images/deed/svg/zero_blue.svg" />';


	foreach ($license_array as &$value) {
		//var_dump($value);


		$img_lic_str = explode("/",$value->URL);

	
		if (strcmp($img_lic_str[4],"by")==0){

                        $img_lic_panel = $cc." ".$by;
			$download_button = TRUE;
                

                }elseif (strcmp($img_lic_str[4],"by-sa")==0){

			$img_lic_panel = $cc." ".$by." ".$sa;
		

		}elseif (strcmp($img_lic_str[4],"by-nd")==0){

                        $img_lic_panel = $cc." ".$by." ".$nd;
                

                }elseif (strcmp($img_lic_str[4],"by-nc")==0){

                        $img_lic_panel = $cc." ".$by." ".$nc;


                }elseif (strcmp($img_lic_str[4],"by-nc-nd")==0){

                        $img_lic_panel = $cc." ".$by." ".$nc." ".$nd;


                }elseif (strcmp($img_lic_str[4],"by-nc-sa")==0){

                        $img_lic_panel = $cc." ".$by." ".$nc." ".$sa;


                }else{

			$img_lic_panel = $cc." ".$zero;
			$download_button = TRUE;
		...
		if($download_button == TRUE){
			...
			//print_r($download_button);
			$download_button = areAllAccessible($id);

		}



		$license = '<a rel="license" href="'.$value->URL.'" target="_blank" >'. $img_lic_panel.'</a>';



		/*$license = '<a rel="license" href="'.$value->URL.'" target="_blank" ><img alt="Creative Commons License" style="border-width:0" src="'.$value->image.'" /></a><a rel="license" target="_blank" href="'.$value->URL.'"></a>';*/
	}
...
//echo $license_text;
$modal_content_license = ' <div class="lightbox_3 hide">
      <div class="lightbox__content">
        <i  class="lightbox__close toggle-lightbox-4 fa fa-times"></i>
      
        <div style="padding-left:20px; width:100%; height:100%; overflow: auto"><center><h1>License</h1></center><span style="padding:10px">'.$license_text.'</span></div>
...
$modal_content = ' <div class="lightbox_2 hide">
      <div class="lightbox__content">
        <i  class="lightbox__close toggle-lightbox-3 fa fa-times"></i>
      
	<div style="padding-left:20px; width:100%; height:100%; overflow: auto"><center><h1>Terms of Use</h1></center>'.$terms_of_use_text.'</div>
...
foreach($providers as &$provider_value){
	...
	$own_space = $provider_value["own_space"];
...
$search_title = $dataset_metadata["dataset"]["title"];

$res_0=searchElastic($search_title);
//print_r($res_0);
$related_publications='<ul class="related">';
...
for($l=0;$l<5;$l++){
	...
	if($res_0["hits"]["total"]>0){
		...
		$publication_id = $res_0["hits"]["hits"][$l]["_source"]["publication_id"];
        	$citation = $res_0["hits"]["hits"][$l]["_source"]["cg.identifier.citation"];
		$title_publication = $res_0["hits"]["hits"][$l]["_source"]["cg.title"];
	...
	if( in_array($publication_id, $double_check)){

	}else if ($publication_id!=0) {

		$list_item= '<li class="related__item">
                	<a style="font-size:21px" href="/publication.php?id='.$publication_id.'" class="related__item--link">'.$title_publication.'</a></li><br>';

		array_push($double_check,$publication_id);

	}

	$related_publications.=$list_item;
...
$files = retrieveFiles($id);

if(!empty($files)){

	$table_files = '<div class="table-responsive"> <table class="table" width="100%"  cellspacing="0"><tbody>';
	$tb_row = '';

	foreach ($files as &$file) {

		$title_file = $file["title"];
		$link = $file["link"];
		...
		if(strcmp($accessibility,'open')==0){
			$img = '<img width="42px" height="42px" src="../img/if_icon-116-lock-open_314389.png">';
			//$check_box = '<td style="border:0px; padding-right:0px; padding-left:0px;padding-bottom:0px; padding-top:25px; width:20px"><input type="checkbox" name="'.$title.'" value="Yes" />';
		}else{

			$img = '<img width="42px" height="42px" src="../img/if_icon-114-lock_314387.png">';
			//$check_box = '<td style="border:0px; padding-right:0px; padding-left:0px;padding-bottom:0px; padding-top:25px; width:20px"><input type="checkbox" name="'.$title.'" value="Yes" disabled />';

		}



if((strcmp(trim($content_type),'text/plain')==0)||(strcmp(trim($content_type),'text/plain; charset=US-ASCII')==0)){
	$fa = "<div style=' color:LightGrey'><i class='fa fa-file-text-o fa-3x'></i></div>";
}elseif(

((strcmp(trim($content_type),'application/octet-stream')==0)||(strcmp(trim($content_type),'application/msaccess')==0))
||((strcmp(trim($content_type),'application/twbx')==0)||(strcmp(trim($content_type),'application/vnd.google-earth.kmz')==0))
||((strcmp(trim($content_type),'application/x-nsdstat')==0)||(strcmp(trim($content_type),'application/x-rlang-transport')==0))
||((strcmp(trim($content_type),'application/x-sas-system')==0)||(strcmp(trim($content_type),'application/x-spss-sav')==0))
||((strcmp(trim($content_type),'application/x-stata')==0)||(strcmp(trim($content_type),'application/x-stata-13')==0))
||((strcmp(trim($content_type),'application/x-stata-syntax')==0)||(strcmp(trim($content_type),'text/html')==0))
||((strcmp(trim($content_type),'text/xml')==0)||(strcmp(trim($content_type),'text/x-spss-syntax; charset=US-ASCII')==0))
||((strcmp(trim($content_type),'text/x-stata-syntax; charset=US-ASCII')==0)||(strcmp(trim($content_type),'text/x-spss-syntax')==0))
||((strcmp(trim($content_type),'text/x-stata-syntax')==0)||(strcmp(trim($content_type),'type/x-r-syntax')==0))
||(strcmp(trim($content_type),'text/x-sas-syntax; charset=US-ASCII')==0)
){
	$fa = "<div style=' color:LightGrey'> <i class='fa fa-file-code-o fa-3x'></i></div>";
}elseif((strcmp(trim($content_type),'application/pdf')==0)||(strcmp(trim($content_type),'application/nappdf')==0)){
		$fa = "<div style=' color:LightGrey'><i class='fa fa-file-pdf-o fa-3x'></i></div>";
}elseif(((strcmp(trim($content_type),'application/msword')==0)||(strcmp(trim($content_type),'application/vnd.ms-word.document.12')==0))||
((strcmp(trim($content_type),'application/vnd.openxmlformats-officedocument.wordprocessingml.document')==0)||(strcmp(trim($content_type),'application/vnd.word')==0))){
		$fa = "<div style=' color:LightGrey'><i class='fa fa-file-word-o fa-3x'></i></div>";
}elseif(

((strcmp(trim($content_type),'application/vnd.ms-excel')==0)||(strcmp(trim($content_type),'application/vnd.ms-excel.sheet.macroEnabled.12')==0))
||((strcmp(trim($content_type),'application/vnd.oasis.opendocument.spreadsheet')==0)||(strcmp(trim($content_type),'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')==0))
||((strcmp(trim($content_type),'text/csv')==0)||(strcmp(trim($content_type),'text/comma-separated-values')==0))
||((strcmp(trim($content_type),'text/tab-separated-values')==0)||(strcmp(trim($content_type),'text/x-fixed-field')==0))

)
{
		$fa = "<div style=' color:LightGrey'><i class='fa fa-file-excel-o fa-3x'></i></div>";
}elseif((strcmp(trim($content_type),'application/vnd.ms-powerpoint')==0)||(strcmp(trim($content_type),'application/vnd.openxmlformats-officedocument.presentationml.presentation')==0)){
		$fa = "<div style=' color:LightGrey'><i class='fa fa-file-powerpoint-o fa-3x'></i></div>";
}elseif((strcmp(trim($content_type),'application/vnd.yellowriver-custom-menu')==0)||(strcmp(trim($content_type),'data/various-formats')==0)){
		 $fa = "<div style=' color:LightGrey'><i class='fa fa-file-o fa-3x'></i></div>";
}elseif(((strcmp(trim($content_type),'application/x-7z-compressed')==0)||(strcmp(trim($content_type),'application/x-gzip')==0))
||((strcmp(trim($content_type),'application/x-rar-compressed')==0)||(strcmp(trim($content_type),'application/x-zip-compressed')==0))
||((strcmp(trim($content_type),'application/zip')==0)||(strcmp(trim($content_type),'application/zipped-shapefile')==0))){
		 $fa = "<div style=' color:LightGrey'><i class='fa fa-file-archive-o fa-3x'></i></div>";
}elseif(

((strcmp(trim($content_type),'image/jpeg')==0)||(strcmp(trim($content_type),'image/png')==0))||(strcmp(trim($content_type),'image/tiff')==0)){
		 $fa = "<div style=' color:LightGrey'> <i class='fa fa-file-image-o fa-3x'></i></div>";
}elseif(strcmp(trim($content_type),'video/avi')==0){
		$fa = "<div style=' color:DarkSeaGreen'> <i class='fa fa-file-video-o fa-3x'></i></div>";
		...
		if((strcmp($accessibility,'open')==0)&&($download_button == TRUE)){


			$tb_row = $tb_row.' <tr><td style="border:0px; padding-right:0px;width:5%">'.$fa.'</td><td style="vertical-align: middle; padding:6px; border:0px ;"> <a href='.$link.'> '.$title_file.'</a><br><span style="font-size:14px">'.$content_type.'</span></td></tr>';


		}else{


			$tb_row = $tb_row.' <tr><td style="border:0px; padding-right:0px;width:5%">'.$fa.'</td><td style="vertical-align: middle; padding:6px; border:0px ;"> '.$title_file.'<br><span style="font-size:14px">'.$content_type.'</span></td></tr>';
	...
	$table_files = $table_files.$tb_row.'</tbody></table></div>';
	$button = '';

	if($download_button == TRUE){
		$button = '<button style="float:right" id="download" class="btn button button-orange">Download</button>';

	}	


	  $dataset_files = '<div class="boxed column small-12 post-content white-bg">
            <h3 class="h1">DATASET FILES</h3> <button id="collapse_button" style="border:0px;float:left; background-color:rgba(0,0,0,.03)"><i class="fa fa-angle-double-down fa-2x" aria-hidden="true"></i></button>
		'.$button.'


                <div class="post-text" id="collapse_dataset_files">'.$table_files.'
                </div>
        </div>';
...
$ontology_list = '<div class="boxed column small-12 post-content white-bg"> <div class="tab">';
$json_agrovoc=retrieveAGROVOC($id);
$json_agro_tech=retrieveAgroTechnology($id);
$json_agronomy=retrieveAgronomy($id);
$json_plant=retrievePlant($id);

$json_ontologies = retieveUnionOntology($id);




//$json_agrovoc = $json_agrovoc.$json_plant.$json_agronomy.$json_agro_tech;
/*print_r($json_agrovoc);
print_r($json_plant);*/
...
if($no_onto == FALSE){
	$ontology_list = $ontology_list.'<button class="tablinks" onclick="openOntology(event,\'agrovoc\')"> <h3 class="h3" style="margin:0px;">Agrovoc</h3></button>'; 

}


$no_onto_agro_tech = empty(json_decode($json_agro_tech));

if($no_onto_agro_tech == FALSE){
        $ontology_list = $ontology_list.'<button class="tablinks" onclick="openOntology(event,\'agricultural\')"> <h3 class="h3" style="margin:0px;">Agricultural Technology</h3></button>';

}

$no_onto_agronomy = empty(json_decode($json_agronomy));

if($no_onto_agronomy == FALSE){
        $ontology_list = $ontology_list.'<button class="tablinks" onclick="openOntology(event,\'agronomy\')"> <h3 class="h3" style="margin:0px;">Agronomy</h3></button>';

}

$no_onto_plant = empty(json_decode($json_plant));

if($no_onto_plant == FALSE){
        $ontology_list = $ontology_list.'<button class="tablinks" onclick="openOntology(event,\'plant\')"> <h3 class="h3" style="margin:0px;">Plant</h3></button>';

}


$ontology_list = $ontology_list.'</div>';
...
$ontology_terms = '';
if($no_onto == FALSE){
        $ontology_terms =  '<div id="agrovoc" class="tabcontent" >
                <div class="post-text">
                        <div id="agrovoc_cloud" style="margin: 0 auto; width: 100%;max-width:700px;height: 190px;"> </div>
                </div>
        </div>';
...
if($no_onto_agro_tech == FALSE){
	$ontology_terms =  $ontology_terms.'<div id="agricultural" class="tabcontent" >
                <div class="post-text">
                        <div id="agrovoc_cloud_2" style="margin: 0 auto; width: 100%;height: 200px;"> </div>
                </div>
        </div>';
...
if($no_onto_agronomy == FALSE){
	...
	$ontology_terms =  $ontology_terms.'<div id="agronomy" class="tabcontent" >
                <div class="post-text">
                        <div id="agrovoc_cloud_3" style="margin: 0 auto; width: 100%;height: 200px;"> </div>
                </div>
        </div>';
...
if($no_onto_agronomy == FALSE){
        ...
        $ontology_terms =  $ontology_terms.'<div id="plant" class="tabcontent" >
                <div class="post-text">
                        <div id="agrovoc_cloud_4" style="margin: 0 auto; width: 100%;height: 200px;"> </div>
                </div>
        </div>';
...
$ontology_list = $ontology_list . $ontology_terms."</div>";
...
<html lang="en">
  <head>
	...
	<script type="text/javascript">var jTerms ='.$json_ontologies.'</script>
	<script type="text/javascript">var jTerms2 ='.$json_agro_tech.'</script>
        <script type="text/javascript">var jTerms3 ='.$json_agronomy.'</script>
        <script type="text/javascript">var jTerms4 ='.$json_plant.'</script>
	<script type="text/javascript">var did ='.$id.'</script>
	...
	<script src="assets/js/accessRightsView.js"></script>
	...
	<script>
	 ...
	 var obj = JSON.parse(JSON.stringify(jTerms, null, 2));
                                var id = "#agrovoc_cloud";



        var data=[];
        //alert(JSON.stringify(jTerms, null, 2));
        ...
        for (var key in obj) {
                if (obj.hasOwnProperty(key)) {
                        ...
                        data[i] = new Object();
                        data[i].text=obj[key].label;

                        data[i].weight=obj[key].frequency;
                        data[i].link=obj[key].URI;
                        i=i+1;
        ...
        $(id).jQCloud(data,{removeOverflowing:true,center: {x: 0.5, y:0.6},   autoResize: true, fontSize: {   from: 0.08,  to: 0.02  }});
...
$body = ' <body>
	...
	<div class="boxed column small-12 post-content white-bg">
            <h3 class="h1">KEYWORDS</h3>
                <div class="post-text">
                        <div id="agrovoc_cloud" style="margin: 0px auto; width: 100%; max-width: 600px; height: 300px; position: relative;" class="jqcloud"> </div>
</div></div>
...
'.$dataset_files.'
          ...
          <div class="boxed column small-12 post-related white-bg">
            <h3 class="h1">RELEVANT PUBLICATIONS</h3>'.$related_publications.'
          </div>
        </div>
      </artilce>
      <!-- end main content -->
	...
	<!-- start post access rights -->
        <div class="boxed post-meta post-meta-access-rights white-bg">
          <h3 class="post-meta__title h2">ACCESS RIGHTS</h3>
          <div class="post-meta__content">
            <p class="access-title"><strong>License</strong></p>
            '.$license.'
            <hr />
            <p class="access-title"><strong>Terms of use</strong></p>
            '.$terms_of_use.'
          </div>
        </div>
