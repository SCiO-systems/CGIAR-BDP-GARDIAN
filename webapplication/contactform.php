...
<html lang="en">
  <head>
    ...
    <title>GARDIAN Support</title>
    <link href="https://fonts.googleapis.com/css?family=Roboto+Condensed:300,300i,400,400i,700,700i&amp;subset=latin-ext" rel="stylesheet">
    <link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css" rel="stylesheet">
     <script src="https://www.google.com/recaptcha/api.js?render=6LdQlqEUAAAAAGa51cz7GVok7rcdOb63YYvEwkrw"></script>
	<script>
	grecaptcha.ready(function() {
      		grecaptcha.execute('6LdQlqEUAAAAAGa51cz7GVok7rcdOb63YYvEwkrw', {action: 'homepage'}).then(function(token) {
...
<body>
    ...
    <main class="main row">
      ...
      <article class="column small-12 row medium-12 main-content ">
		...
		<div class="boxed white-bg" ><h3 class="h2">GARDIAN SUPPORT FORM</h3>

			<form name="contact_form" action="formprocess.php" id="comment_form" method="POST">  

				<label>Your name (only if you’d like us to know)</label>
					<input name="name" type="text" id="name" size="55"><br>
				<label>Your email (only if you’d like a response)</label>
					<input name="email" type="text" id="email" size="55"><br>
				<label>Subject</label>  
					<input name="subject" type="text" id="subject" size="55"><br>  
				<label>Message</label>  
					<textarea name="message" id="message" rows="4" cols="40"></textarea><br>  
<!-- [PUT THE CAPTCHA WHERE YOU WANT IT] -->
					<input type="submit" id="submit" value="Send Message" class="btn button button-orange">  
			</form>  
	<script>
       // when form is submit
	...
	var name = $("#name").val();
        ...
        grecaptcha.ready(function() {
            ...
            grecaptcha.execute('6LdQlqEUAAAAAGa51cz7GVok7rcdOb63YYvEwkrw', {action: 'create_comment'}).then(function(token) {
                // add token to form
                $('#comment_form').prepend('<input type="hidden" name="g-recaptcha-response" value="' + token + '">');
                    $.post("formprocess.php",{subject: subject, message: message,token: token,name: name,email: email}, function(result) {
                            console.log(result);
                            /*if(result.success) {
